...
def generate_poly(x, y, radius, num_vertices=5):
    points = np.linspace(0, 2 * np.pi, num_vertices)
    return [(radius * np.cos(t) + x, radius * np.sin(t) + y) for t in points[:-1]]


class ORCA_Obstacles(Policy):
    def __init__(self, config):
        ...
        for obs in self.config.obstacles:
            self.obstacles.append(generate_poly(obs.px, obs.py, obs.radius))
